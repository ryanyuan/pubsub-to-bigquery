...
public class JsonMsgToTableRowWithRecord
        extends PTransform<PCollection<PubsubMessage>, PCollection<TableRow>> {

    private static final Logger LOGGER = LoggerFactory.getLogger(JsonMsgToTableRowWithRecord.class);
                ...
                new SimpleFunction<PubsubMessage, TableRow>() {
                    ...
                    public TableRow apply(PubsubMessage message) {
                        ...
                        try {
                            ...
                            TableRow nestedGeo = new TableRow();
                            JSONObject geo = jsonObj.getJSONObject(FIELD_GEO);
                            nestedGeo.set(FIELD_LAT, geo.getFloat(FIELD_LAT));
                            nestedGeo.set(FIELD_LON, geo.getFloat(FIELD_LON));
                            row.set(FIELD_GEO, nestedGeo);
    ...
    public static TableSchema getSchema() {
        ...
        fields.add(new TableFieldSchema().setName(FIELD_GEO).setType(TYPE_RECORD).setFields(
                new ArrayList<TableFieldSchema>() {
                    {
                        add(new TableFieldSchema().setName(FIELD_LAT).setType(TYPE_FLOAT));
                        add(new TableFieldSchema().setName(FIELD_LON).setType(TYPE_FLOAT));
                    }
                }
        ));
